...
class Archiver {
	...
	/**
	 * To archive only favorited tweets
	 *
	 * @return string Returns a string with informational output.
	 */
	public function archiveFavorites()
	{
		...
		while ($gotResults) {
			...
			try {
				...
				if ($numResults == 0) {
					$str .= 'NO favorite tweets on page ' . $page . ", exiting.\n";
					$gotResults = false;
				} else {
					...
					$str .= $numResults . ' favorite tweets on page ' . $page . " (oldest: ".$oldestTweet['id'].", newest: ".$newestTweet['id'].")\n";
					...
					if ($result === false) {
						$str .= 'ERROR INSERTING FAVORITE TWEETS INTO DATABASE: ' . $this->model->getLastErrorMessage() . "\n";
					} else if ( $result == 0 ) {
						$str .= 'Zero favorite tweets added.' . "\n";
					} else {
						$str .= $result . ' favorite tweets added.' . "\n";
						$numAdded += $result;
					}
